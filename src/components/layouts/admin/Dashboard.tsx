...
const Dashboard = ({ children }: LayoutProps) => {
  ...
  useEffect(() => {
    ...
    ContactAdmin.handleGetList(body)
      .then((result: any) => {
        // Xử lý kết quả trả về ở đây
        const { data, meta } = result;
        if (meta.status === 200) {
  ...
  useEffect(() => {
    ...
    if (api) {
      ...
      socket.on("orderCreated", (order) => {
        ...
        showNotification(
          "Hệ thống: Bạn có đơn hàng mới",
          `${order.name} đã tạo một đơn hàng mới`,
          logoCompany
        );
        const orderPush = [
          {
      ...
      socket.on("newContact", (contact) => {
        ...
        // Xử lý thông tin đơn hàng ở đây
        showNotification(
          "Hệ thống: Bạn có liên hệ mới",
          `${contact.name} đã gửi cho bạn một liên hệ mới`,
          logoCompany
        );
        const contactPush = [
          {
        ...
        <>
          ...
          <div className="admin__notice-title-sub">
            {totalNotice > 0 ? (
              <>
                Bạn có{" "}
                <span className="admin__notice-title-sub-highlight">
                  {totalNotice}
                </span>{" "}
                thông báo chưa đọc
              </>
              ...
              >
                <div className="admin__notice-item-information">
                  <div className="admin__notice-item-information-header">
                    {item.isOrder ? (
                      <ShoppingCartOutlined className="admin__notice-item-information-header-icon" />
                    ) : (
                      <MailOutlined className="admin__notice-item-information-header-icon" />
                    )}
