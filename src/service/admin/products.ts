...
const handleGetHighlight = async (): Promise<ResponseData | Error> => {
  try {
    const response = await Products.getHighlight();
    const { data, meta } = response.data;
    if (meta.status === 200) {
      const resData: ResponseData = { meta: meta, data: data };
      return resData;
...
export default { handleCreateProducts, handleUpdateProducts, handleUploadImageProducts, handleGetAllProducts, handleGetOne, handleUpdateStatus, handleGetHighlight};
